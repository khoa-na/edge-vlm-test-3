...
- **Tier 1 (mọi frame, <50ms, CPU/NPU):** MediaPipe FaceLandmarker (478 điểm) tính **EAR** (nhắm mắt, ngưỡng 0.20), **MAR** (ngáp, ngưỡng 0.35 — hiệu chỉnh trên dataset FL3D), **PERCLOS** (% nhắm mắt cửa sổ 60s), **Head Pose Euler** (pitch < −25° = cúi nhìn điện thoại, |yaw| > 45° = quay đầu), **YOLO26n INT8** phát hiện điện thoại — chạy mọi frame vì là sự kiện khẩn cấp <300ms; chọn YOLO26n vì NMS-free/DFL-free nên latency tất định và không rớt accuracy khi quantize INT8. Trên các chỉ số thô là **temporal state machine**: debounce (EAR thấp ≥15 frame liên tục mới là "nhắm mắt", phone cần 3 frame xác nhận), cửa sổ trượt, cooldown. Cảnh báo khẩn cấp (T0/T1) phát trực tiếp bằng TTS tĩnh duyệt sẵn — **không bao giờ chờ VLM**.
